...
--data table without returned products, NULL customerID and InvoiceDate in Date format
...
--RFM ANALYSIS
WITH CTE AS --calculate the recency, frequency and monetary
(
	SELECT
			CustomerID, 
	...
	CTE2 AS --apply Quantile concepts to segment the customers
(
	SELECT 
			CustomerID,
			last_active_date,
	...
	CTE3 AS --label for the quantiles of recency, frequency and monetary by 1,2,3,4
(
	SELECT
			CustomerID,
			last_active_date,
	...
	CTE4 AS --calculate sum of Recency_rank, Frequency_rank and Monetary_rank then calculate the quantile of RFM to segment the customers
(
	SELECT 
			CustomerID,
			last_active_date,
